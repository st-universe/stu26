...
if ($v == "main")
{
	...
	<th width=30></th><th>Name</th><th width=30></th><th>System</th><th>Sektor</th><th>Energie</th><th>Warpkern</th><th>Lager</th>";
	...
	else
	{
		while($data=mysql_fetch_assoc($list))
		{
			...
			$endata = $sta->loadenergydatabyid($data[id]);
			...
			<td>".$data['eps']."/".$data['max_eps']." (".($endata[egesamt] >= 0 ? "<font color=#30BF00><b>+".$endata[egesamt]."</b></font>" : "<font color=#C40005><b>".$endata[egesamt]."</b></font>").")</td>
			<td>".$data['wkload']."/".$data['wkload_max']." (".($endata[wkplus] >= 0 ? "<font color=#30BF00><b>+".$endata[wkplus]."</b></font>" : "<font color=#C40005><b>".$endata[wkplus]."</b></font>").")</td>
			<td>";
		...
		if (count($arr) > 0)
		{
			foreach($arr as $key => $data)
			{
				...
				if ($lf != $data['fleets_id'] && $data['fleets_id'] > 0)
				{
					...
					echo "<td colspan=8>".($data['user_id'] == $_SESSION['uid'] ? "<a href=?p=ship&s=ss&id=".$data['fship_id'].">".$defadd."".stripslashes($data['fname'])."</a>" : stripslashes($data['fname']))."</td>";
					$lf = $data['fleets_id'];
				}
				if ($lf != $data['fleets_id'] && $data['fleets_id'] == 0 && $data['slots'] == 0)
				{
					echo "<td colspan=8>Einzelschiffe</td>";
					$lf = 0;
				}
				...
				echo "<td align=center>B</td></tr>";

			}
		}
		if ($cl > 0) echo "<tr><td colspan=8>Es befinden sich nicht scanbare Objekte in diesem Sektor</td></tr>";
		echo "</table>";
