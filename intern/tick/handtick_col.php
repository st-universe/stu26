...
function deactivate_organik(&$res)
{
	...
	for($k=1;$k<=$res;$k++)
	{
		$rd = $db->query("SELECT a.field_id,b.buildings_id,b.name,b.bev_use,b.bev_pro,b.eps_proc,b.research_t,b.research_k,b.research_v,b.reseach_d FROM stu_colonies_fielddata as a LEFT JOIN stu_buildings as b USING(buildings_id) WHERE a.colonies_id=".$data['id']." AND a.buildings_id=103 AND a.aktiv=1 LIMIT 1",4);
		if ($rd == 0)
		{
			if ($good == 1)
			{
...
$log->deleteLogType("tickcol_".$group);
$log->enterLog("tickcol_".$group,"start");
...
if ($group == 0) $result = $db->query("SELECT a.id,a.name,a.colonies_classes_id,a.user_id,a.bev_free,a.bev_work,a.bev_max,a.eps,a.max_eps,a.schilde,a.max_schilde,a.max_storage,a.bevstop,a.einwanderung,a.gravitation,b.is_moon,c.lav_not FROM stu_colonies as a LEFT JOIN stu_colonies_classes as b USING(colonies_classes_id) LEFT JOIN stu_user as c ON a.user_id=c.id WHERE a.user_id!=1 AND ISNULL(c.vac_active) ORDER BY a.user_id,a.id");
else $result = $db->query("SELECT a.id,a.name,a.colonies_classes_id,a.user_id,a.bev_free,a.bev_work,a.bev_max,a.eps,a.max_eps,a.schilde,a.max_schilde,a.max_storage,a.bevstop,a.einwanderung,a.gravitation,b.is_moon,c.lav_not FROM stu_colonies as a LEFT JOIN stu_colonies_classes as b USING(colonies_classes_id) LEFT JOIN stu_user as c ON a.user_id=c.id WHERE a.user_id!=1 AND ISNULL(c.vac_active) AND c.tick=".$group." ORDER BY a.user_id,a.id");
while($data=mysql_fetch_assoc($result))
{
	$log->enterLog("tickcol_".$group,"start col ".$data['id']);
	...
	if ($red['rk'] > 0 || $red['rt'] > 0 || $red['rv'] > 0 || $red['rd'] > 0)
	{
...
if ($msg != "") $db->query("INSERT INTO stu_pms (send_user,recip_user,text,date,type) VALUES ('1','".$luser."','".$msg."',NOW(),'4')");
	...
	$log->enterLog("tickcol_".$group,"done");
	
// @unlink("/var/tmp/lock_coltick_".$group);
?>
