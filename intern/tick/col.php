...
function deactivate(&$good)
{
	...
	// }
	if ($rd['buildings_id'] == 54) $gd[1] -= 2*$db->query("SELECT COUNT(field_id) FROM stu_colonies_fielddata WHERE (buildings_id=2 OR buildings_id=9) AND aktiv=1 AND colonies_id=".$data['id'],1);
...
function deactivate_nonresearched() {
	...
	while($d=mysql_fetch_assoc($result))
	{
		...
		$data[bev_free] += $bdata[bev_use];
	}
}

function deactivate_livingroom() {
	global $data,$db,$smsg;

	$result = $db->query("SELECT a.*,b.bev_use FROM stu_colonies_fielddata as a left join stu_buildings as b on a.buildings_id = b.buildings_id WHERE a.buildings_id > 0 AND a.aktiv = 1 AND a.colonies_id = ".$data[id]." AND b.bev_use > 0 ORDER by bev_use DESC LIMIT 1;");
	while($d=mysql_fetch_assoc($result))
	{
		$bdata = $db->query("SELECT * FROM stu_buildings WHERE buildings_id = ".$d[buildings_id]." LIMIT 1;",4);
		$db->query("UPDATE stu_colonies_fielddata SET aktiv = '0' WHERE colonies_id = ".$d[colonies_id]." AND field_id = ".$d[field_id]." LIMIT 1;",4);		
		$smsg .= $bdata[name]." (Feld ".$d[field_id].") wegen Wohnraummangel deaktiviert<br>";
		$data[bev_work] -= $bdata[bev_use];
		$data[bev_free] += $bdata[bev_use];
	}
}
...
while($data=mysql_fetch_assoc($result))
{
	...
	if ($data[bev_work] > $data[bev_max])
		deactivate_livingroom();
	...
	// Wetterkontrolle
	if ($db->query("SELECT field_id FROM stu_colonies_fielddata WHERE buildings_id=54 AND aktiv=1 AND colonies_id=".$data['id'],1) != 0) $gd[1] += 2 * $db->query("SELECT COUNT(field_id) FROM stu_colonies_fielddata WHERE (buildings_id=2 OR buildings_id=9) AND aktiv=1 AND colonies_id=".$data['id'],1);
