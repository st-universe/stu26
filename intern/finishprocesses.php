...
while($data=mysql_fetch_assoc($result))
{
	...
	$aktiv = 0;
	$stop = 0;
	if ($bd['needs_rbf'] == 1 && $db->query("SELECT field_id FROM stu_colonies_fielddata WHERE aktiv=1 AND colonies_id=".$data['colonies_id']." AND buildings_id=19 LIMIT 1",1) == 0) $stop = 1;
	...
	if (!$data['value'] && $bd['is_activateable'] == 1 && $bf >= $bd['bev_use'] && $stop == 0 && $data['buildings_id'] != 100)
	{
...
while($data=mysql_fetch_assoc($result))
{
	...
	{
		$db->query("UPDATE stu_stations SET bev_free=bev_free-".$bd['bev_use'].",bev_work=bev_work+".$bd['bev_use'].",bev_max=bev_max+".$bd['bev_pro']." WHERE id=".$data['stations_id']." LIMIT 1");
		$aktiv = 1;
		$bf -= $bd['bev_use'];
	}
	$txt .= $bd['name']." auf Feld ".$data['field_id']."<br>";
	$db->query("UPDATE stu_stations SET max_storage=max_storage+".$bd['lager'].",max_eps=max_eps+".$bd['eps'].",max_schilde=max_schilde+".$bd['schilde']." WHERE id=".$data['stations_id']." LIMIT 1");
...
while($data=mysql_fetch_assoc($result))
{
	if ($data['ships_id'] > 0)
	{
		$ship = $db->query("SELECT systems_id,sx,sy,cx,cy FROM stu_ships WHERE id=".$data['ships_id']." LIMIT 1",4);
		$comm->sendpm(1,$data['user_id'],"Der Stationsbau in Sektor ".($ship['systems_id'] > 0 ? $ship['sx']."|".$ship['sy']." (".$db->query("SELECT name FROM stu_systems WHERE systems_id=".$ship['systems_id']." LIMIT 1",1)." System)" : $ship['cx']."|".$ship['cy'])." wurde abgeschlossen",4);
		$rump = $db->query("SELECT min_crew,max_crew FROM stu_rumps WHERE rumps_id=".$data['rumps_id']." LIMIT 1",4);
		$res = $db->query("INSERT INTO stu_ships (user_id,rumps_id,plans_id,systems_id,cx,cy,sx,sy,direction,name,warpable,cloakable,
		max_eps,max_batt,huelle,max_huelle,max_schilde,lss_range,kss_range,max_crew,min_crew,phaser,cfield,points) VALUES ('".$data['user_id']."',
		'".$data['rumps_id']."','".$data['plans_id']."','".$ship['systems_id']."','".$ship['cx']."','".$ship['cy']."','".$ship['sx']."','".$ship['sy']."',
		'1','Schiff','".($data['warpable'] == 1 ? 1 : '')."','".($data['cloakable'] == 1 ? 1 : '')."','".$data['eps']."','".$data['eps']."','".$data['huelle']."','".$data['huelle']."','".$data['schilde']."','".$data['lss']."',
		'".$data['kss']."','".$rump['max_crew']."','".$rump['min_crew']."','".$data['phaser']."','".$db->query("SELECT type FROM stu_sys_map WHERE systems_id=".$ship['systems_id']." AND sx=".$ship['sx']." AND sy=".$ship['sy']." LIMIT 1",1)."','".$data['points']."')",5);
		...
		$db->query("DELETE FROM stu_ships WHERE id=".$data['ships_id']." LIMIT 1");
		$db->query("DELETE FROM stu_ships_storage WHERE ships_id=".$data['ships_id']);
		$db->query("UPDATE stu_ships SET traktormode=0,traktor=0 WHERE traktor=".$data['ships_id']." LIMIT 1");
		$db->query("UPDATE stu_ships SET dock=".$res." WHERE dock=".$data['ships_id']);
		$db->query("DELETE FROM stu_ships_ecalls WHERE ships_id=".$data['ships_id']." LIMIT 1");
		continue;
	}
	$comm->sendpm(1,$data['user_id'],"In der Werft im Orbit der Kolonie ".$data['name']." wurde ein Schiff fertiggestellt",4);
	$rump = $db->query("SELECT min_crew,max_crew FROM stu_rumps WHERE rumps_id=".$data['rumps_id']." LIMIT 1",4);
	$res = $db->query("INSERT INTO stu_ships (user_id,rumps_id,plans_id,systems_id,cx,cy,sx,sy,direction,name,warpable,cloakable,
	max_eps,max_batt,huelle,max_huelle,max_schilde,lss_range,kss_range,max_crew,min_crew,phaser,cfield,points,lastmaintainance) VALUES ('".$data['user_id']."',
	'".$data['rumps_id']."','".$data['plans_id']."','".$data['systems_id']."','".$data['cx']."','".$data['cy']."','".$data['sx']."','".$data['sy']."',
	'1','Schiff','".($data['warpable'] == 1 ? 1 : '')."','".($data['cloakable'] == 1 ? 1 : '')."','".$data['eps']."','".$data['batt']."','".$data['huelle']."','".$data['huelle']."','".$data['schilde']."','".$data['lss']."',
	'".$data['kss']."','".$rump['max_crew']."','".$rump['min_crew']."','".$data['phaser']."','".$db->query("SELECT type FROM stu_sys_map WHERE systems_id=".$data['systems_id']." AND sx=".$data['sx']." AND sy=".$data['sy']." LIMIT 1",1)."','".$data['points']."','".$time."')",5);
	$db->query("INSERT INTO stu_ships_logdata (ships_id,user_id,name,buildtime) VALUES ('".$res."','".$data['user_id']."','Schiff','".$time."')");
	$db->query("DELETE FROM stu_ships_buildprogress WHERE buildtime=".$data['buildtime']." AND plans_id=".$data['plans_id']." AND rumps_id=".$data['rumps_id']." AND user_id=".$data['user_id']." LIMIT 1");
	if ($data['plans_id'] == 1) $db->query("UPDATE stu_ships SET warpcore=500 WHERE id=".$res." LIMIT 1");
}
// Perimeteralarm SYS
...
while($data=mysql_fetch_assoc($result))
{
	if ($db->query("SELECT systems_id FROM stu_systems_user WHERE systems_id=".$data['systems_id']." AND user_id=".$data['user_id']." LIMIT 1",1) > 0) continue;
	$comm->sendpm(1,$data['user_id'],"Die ".$data['name']." hat die Kartographierung des Systems abgeschlossen",3);
	$db->query("INSERT INTO stu_systems_user (systems_id,user_id) VALUES ('".$data['systems_id']."','".$data['user_id']."')");
}
