...
<tr>
	...
	if ($field['component_id'] > 0)
	{
		...
		echo "<td style=\"width: 130px; border : 1px solid #9C1417; margin-left: 3px;\"><a href=\"javascript:void(0);\" onClick=\"showConfirm(".$_GET['fid'].");\"><img src=".$gfx."/buttons/demont.gif border=0><font color=#FF0000> Deinstallieren</font></a></span></td>";

		if ($field['component_id'] == 120) echo "<td style=\"width: 200px; border : 1px solid #502323; margin-left: 3px;\"><a href=?p=station&s=torp&id=".$_GET['id']."&fid=".$_GET['fid']."><img src=".$gfx."/buttons/torp.gif border=0> Torpedos herstellen</a></span></td>";

		if ($field['component_id'] == 71) $torpclass = 1;
		if ($field['component_id'] == 72) $torpclass = 2;
		if ($field['component_id'] == 73) $torpclass = 3;
		if ($field['component_id'] == 74) $torpclass = 4;
		if ($field['component_id'] == 75) $torpclass = 2;
		if ($field['component_id'] == 76) $torpclass = 2;

		if ($torpclass != 0 && $field['aktiv'] == 1) {

			echo "</tr>";
			$torps = $db->query("SELECT a.* FROM stu_torpedo_types as a LEFT JOIN stu_stations_storage as b on a.goods_id = b.goods_id WHERE b.stations_id=".$_GET['id']." AND b.count > 0 AND a.type <= ".$torpclass." ORDER by a.type DESC");

			while($tdata=mysql_fetch_assoc($torps))
			{
				$border = "502323";
				if ($tdata[torp_type] == $field[torptype]) $border = "20FF20";
				echo "<td style=\"width: 300px; border : 1px solid #".$border."; margin-left: 3px;\"><a href=?ps=".$_SESSION['pagesess']."&p=station&s=show&id=".$_GET['id']."&a=ltsmash&fid=".$_GET['fid']."&tt=".$tdata[torp_type]."><img src=".$gfx."/goods/".$tdata[goods_id].".gif border=0> ".$tdata[name]."</a></td></tr>";
			}
			echo "<tr>";
		}
...
<tr>
	...
	if ($field['component_id'] > 0)
	{
		echo "<img src=".$gfx."/components/".$field['type']."_".$field['component_id'].".gif title=\"".ftit($field['name'])."\"><br />";
		$building = $db->query("SELECT * FROM  stu_station_components WHERE component_id=".$field['component_id']." LIMIT 1",4);
		if ($building['is_activateable'] == 1)
		{
			echo "<br /><b>Produktion</b>";
...
<tr><td style=\"border : 1px solid #262323; width: 180px;\"><a href=?p=station&s=show&a=free&id=".$_GET['id']."&fid=".$_GET['fid']." ".getonm('x','buttons/x')."><img src=".$gfx."/buttons/x1.gif border=0 name=x title=\"Freigeben\"> Schiff freistellen</a></td></tr></table></td>";
	}
	echo "<div id=dmc></div>
	</td>
	</tr>
