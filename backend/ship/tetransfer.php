...
$tar = $db->query("SELECT id,name,sx,sy,systems_id FROM stu_stations WHERE id=".$_GET['t']." LIMIT 1",4);
if ($tar == 0) exit;
...
echo "<form action=main.php method=get name=\"pform\"><input type=hidden name=p value=ship>
<table class=\"tcal\" style=\"border: 1px solid #8897cf;\">
<input type=hidden name=s value=ss><input type=hidden name=a value=ett>
