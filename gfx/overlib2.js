...
registerCommands('puid,idalert,capalign,textalign');
////////
...
if (typeof ol_idalert=='undefined') var ol_idalert=1;
...
function parseSecondPUExtras(pf,i,ar){
	...
	if (k < ar.length) {
		...
		if (ar[k]==IDALERT) { eval(pf +'idalert=('+pf+'idalert==1) ? 0 : 1'); return k; }
...
function dispSecondPU(theDiv) {
	...
	over2=(olNs4 ? o3_frame.document.layers[theDiv] : (document.all ? o3_frame.document.all[theDiv] : o3_frame.document.getElementById(theDiv)));
	if (olNs4&&typeof over2=='undefined') {
		if (o3_idalert) alert('DIV container "'+theDiv+'" is not defined!!');
		return;
	} else if (!over2) {
		over2=o3_frame.document.createElement("DIV");
		over2.id = theDiv;
		var body = (olIe4 ? o3_frame.document.all.tags('BODY')[0] : o3_frame.document.getElementsByTagName("BODY")[0]);
		body.insertBefore(over2,body.firstChild);
	}
...
function getDivZindex(id) {
	...
	obj=(olNs4 ? o3_frame.document.layers[id] : (document.all ? o3_frame.document.all[id] : o3_frame.document.getElementById(id)));
...
if (olInfo.meets(4.14)) registerNoParameterCommands('idalert');
}
